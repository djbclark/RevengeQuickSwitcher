...
|----|--------|------|-------------|-------|
| **A1** | Device QA | **Low** (process) | Requires a Revenge Discord client and manual checklist | **v4.5.2**. Retest: `/servers` opens **top-docked** switcher (not covered by keyboard) with Prev/Next paging; ambiguous query opens tappable picks; **Open switcher** in settings; fallback bot list if sheet missing; real guild NAV. |
...
| **B17** | Switcher sheet + tappable picks (C5/C8) | **Done** (v4.5.2) | **Medium** | `openLazy` sheet + simple action sheet; bot-message fallback. |
...
**Status:** **Done** (v4.5.2) — `/servers` opens a searchable action sheet; settings has **Open switcher**; bot-message list remains fallback when sheet APIs are missing.
